...
public function get_rows($start = 0) { 
	...
	// Get rows to display
	$rows = DB::query("SELECT * FROM coin_overpayments ORDER BY date_added DESC LIMIT $start,$this->rows_per_page");
	...
	foreach ($rows as $row) { 
		...
		$row['amount'] = fmoney_coin($row['amount_btc']) . ' BTC';
		$row['checkbox'] = "<center><input type=\"checkbox\" name=\"overpayment_id[]\" value=\"$row[id]\"></center>";
		...
		$row['viewtx'] = "<center><a href=\"" . SITE_URI . "/admin/financial/tx?txid=$irow[txid]\" class=\"btn btn-primary btn-xs\">View Tx</a></center>";
